...
const TAG_CATEGORY_MAP: Record<string, string> = {
  ...
  web: "تطوير المواقع",
  react: "تطوير المواقع",
  nextjs: "تطوير المواقع",
  vue: "تطوير المواقع",
  angular: "تطوير المواقع",
  node: "تطوير المواقع",
  nodejs: "تطوير المواقع",
  backend: "تطوير المواقع",
  frontend: "تطوير المواقع",
  javascript: "جافاسكريبت",
  typescript: "جافاسكريبت",
  js: "جافاسكريبت",
  ts: "جافاسكريبت",
  css: "تصميم UI/UX",
  ux: "تصميم UI/UX",
  ui: "تصميم UI/UX",
  design: "تصميم UI/UX",
  tailwind: "تصميم UI/UX",
  tailwindcss: "تصميم UI/UX",
  figma: "تصميم UI/UX",
  mobile: "تطبيقات الجوال",
  reactnative: "تطبيقات الجوال",
  flutter: "تطبيقات الجوال",
  android: "تطبيقات الجوال",
  ios: "تطبيقات الجوال",
  swift: "تطبيقات الجوال",
...
function getCategory(tags: string[]): string {
  for (const tag of tags) {
    ...
    const cat = TAG_CATEGORY_MAP[normalized];
    if (cat) return cat;
  }
...
/* ─── Featured Card ─────────────────────────────────────── */
function FeaturedCard({ article }: { article: DevArticle }) {
        ...
        <div className="p-6">
          <div className="flex flex-wrap items-center gap-3 mb-3 text-xs text-[#8ba3c7]">
            ...
            <span className="flex items-center gap-1">
              <Clock size={11} />
              {article.reading_time_minutes} دقائق للقراءة
            </span>
          </div>
          ...
          {article.description && (
            <p className="text-[#8ba3c7] leading-relaxed line-clamp-2">{article.description}</p>
          )}
...
/* ─── Grid Card ─────────────────────────────────────────── */
function ArticleCard({ article }: { article: DevArticle }) {
  const category = getCategory(article.tag_list);
  const color = CATEGORY_COLORS[category] ?? "#00d4aa";
  const href = article.canonical_url || article.url;
    ...
    <a href={href} target="_blank" rel="noopener noreferrer" className="group block h-full">
      <div className="h-full bg-[#161b22] border border-white/5 rounded-2xl overflow-hidden hover:border-[#00d4aa]/20 hover:-translate-y-1 transition-all duration-300">
        ...
        <div className="p-5">
          ...
          {article.description && (
            <p className="text-[#8ba3c7] text-xs leading-relaxed line-clamp-3">{article.description}</p>
          )}
